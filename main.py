...
# ===================== STORAGE =====================
# Single source of truth — resolved ONCE at startup, never changes after that.
def _resolve_data_file() -> str:
    """Find a writable directory and return the full path to wallet_data.json.
    Called once at import time so every function uses the exact same path."""
    candidates = [
        "/data",
    ...
    raise RuntimeError("❌ No writable directory found for data storage!")

# Resolved once at startup — NEVER changes
DATA_FILE: str = _resolve_data_file()
DATA_DIR: str = os.path.dirname(DATA_FILE)
DATA_DIR_WRITABLE: bool = True  # guaranteed by _resolve_data_file
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        text = (
            ...
            f"💾 File exists: `{os.path.exists(DATA_FILE)}`\n\n"
            "Data will survive redeploys as long as Railway Volume is mounted at `/data`."
        )
        await query.edit_message_text(text=text, reply_markup=back_kb, parse_mode=ParseMode.MARKDOWN)
        return
...
async def _post_init_storage_check(app):
    ...
    print(f"✅ Storage OK — using {DATA_FILE}")
    for aid in ADMIN_IDS:
        try:
