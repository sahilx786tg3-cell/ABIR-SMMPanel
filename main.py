...
# ===================== CONFIG =====================
BOT_TOKEN = "8704244305:AAFRGv42Rykc37sFrPp5RFjquWehxqO-H2c"
...
def _finalize_deposit(user_id: int, usd_amount: float, tx_hash: str, label: str, token_amount: float):
    """Credits the user's wallet and records the transaction atomically in ONE lock.
    Previously this used get_user() + update_user() + a second load_data(), which caused
    a race condition: the second load could overwrite the balance just written by update_user(),
    silently losing the deposit credit. Now everything happens inside a single _DATA_LOCK block."""
    with _DATA_LOCK:
        data = load_data()
        uid = str(user_id)
        ...
        # Credit balance directly — no separate get_user/update_user calls
        data["users"][uid]["balance"] = round(data["users"][uid]["balance"] + usd_amount, 6)
        # Record transaction
        data["transactions"].append({
            "user_id": user_id,
            "amount": usd_amount,
        ...
        save_data(data)
        print(f"✅ Deposit finalized: user={user_id} amount=${usd_amount:.4f} hash={tx_hash[:20]}...")
...
async def adminpanel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    # Always clear any stuck state when admin opens the panel fresh
...
async def admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NOTE: Do NOT clear admin_action here — clear it only after successful processing.
    # Clearing before try/except meant any typo (e.g. bad price) wiped the state and
    # forced admin to restart from /adminpanel every single time.
    ...
    try:
        if action.startswith("set_text:"):
            ...
            if text_in.strip().upper() == "RESET":
                ...
                set_text(key, update.message.text)
                context.user_data["admin_action"] = None
            ...
            with _DATA_LOCK:
                ...
                if uid not in data["users"]:
                    data["users"][uid] = {
                        "balance": 0.0, "orders_placed": 0, "orders": [],
                        "total_spending": 0.0, "banned": False, "referred_by": None,
                        "referrals": [], "total_earned": 0.0, "username": None, "first_name": "",
                    }
                data["users"][uid]["balance"] = round(data["users"][uid]["balance"] + amount, 6)
                new_bal = data["users"][uid]["balance"]
                save_data(data)
            context.user_data["admin_action"] = None
            await update.message.reply_text(
                f"➕ Added `${amount:.2f}` to user `{target_id}`. New balance: `${new_bal:.3f}`",
                parse_mode=ParseMode.MARKDOWN,
            )
            ...
            context.user_data["admin_action"] = None
            await update.message.reply_text(
                f"➖ Subtracted `${amount:.2f}` from user `{target_id}`. New balance: `${new_bal:.3f}`",
                parse_mode=ParseMode.MARKDOWN,
            )
            ...
            context.user_data["admin_action"] = None  # always clear after search
            if err:
                await update.message.reply_text(
            ...
            if not new_service:
                ...
                try:
                    ...
                    # Keep state alive — don't clear admin_action — let them retry
                    await update.message.reply_text(
                        f"⚠️ `{text_in}` is not a valid price. Send a number like `1.50` or `0.24`.",
                        parse_mode=ParseMode.MARKDOWN,
                    )
                    return
                ...
                await update.message.reply_text(
                    ...
                    "_Tip: type in lowercase, e.g. `telegram` not `Telegram`_",
                    parse_mode=ParseMode.MARKDOWN,
                )
            ...
            if not new_service:
                ...
                platform = text_in.strip().lower()  # accept Telegram, TELEGRAM, telegram — all work
                if platform not in PLATFORM_KEYS:
                    # Keep state alive so they can retry without restarting
                    platform_list = ", ".join(PLATFORM_KEYS)
                    await update.message.reply_text(
                        f"⚠️ `{text_in}` is not valid. Reply with exactly one of:\n`{platform_list}`",
                        parse_mode=ParseMode.MARKDOWN,
                    )
                else:
...
async def order_quantity_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "order" in result:
        ...
        with _DATA_LOCK:
            data = load_data()
            uid = str(user_id)
            data["users"][uid]["balance"] = round(data["users"][uid]["balance"] - price, 6)
            data["users"][uid]["orders_placed"] = data["users"][uid].get("orders_placed", 0) + 1
            data["users"][uid]["total_spending"] = round(data["users"][uid].get("total_spending", 0.0) + price, 6)
            data["users"][uid].setdefault("orders", []).append({
                "id": order_id,
            ...
            referrer_id = data["users"][uid].get("referred_by")
            if referrer_id:
                commission = round(price * COMMISSION_RATE, 6)
                ref_uid = str(referrer_id)
                if ref_uid in data["users"]:
                    data["users"][ref_uid]["balance"] = round(data["users"][ref_uid]["balance"] + commission, 6)
                    data["users"][ref_uid]["total_earned"] = round(data["users"][ref_uid].get("total_earned", 0.0) + commission, 6)
            save_data(data)
...
async def _post_init_storage_check(app):
    ...
    if DATA_DIR_WRITABLE:
        ...
        warning = (
            ...
            f"`{DATA_DIR}` is *NOT writable* on this deploy. No Railway Volume is mounted.\n\n"
            "⚠️ *This is why user balances reset to 0 and deposits stop working after redeploy.*\n\n"
            "Every redeploy wipes all data (balances, orders, processed_tx) — and old deposits "
            "can get re-credited because `processed_tx` was also wiped.\n\n"
            "✅ *Fix (takes 2 minutes):*\n"
            "Railway → your Service → *Settings* → scroll to *Volumes* → "
            "*+ New Volume* → set mount path to exactly `/data` → Save → Redeploy.\n\n"
            "After that, balances will survive redeploys permanently."
        )
        print(warning)
        try:
            for aid in ADMIN_IDS:
