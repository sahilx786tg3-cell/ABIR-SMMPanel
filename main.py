...
# ===================== CONFIG =====================
BOT_TOKEN = "8704244305:AAHIWu-7i01M7g72xwJXtIw2x8_s63yyk0I"
...
# Fallback paths if /data is not writable (Railway without Volume, local dev, etc.)
_FALLBACK_DIRS = [
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "data"),  # ./data/ next to bot file
    os.path.expanduser("~/bot_data"),  # home directory
    "/tmp/bot_data",  # last resort
]
...
def _ensure_data_dir():
    global DATA_DIR_WRITABLE, DATA_DIR, DATA_FILE
    if DATA_DIR_WRITABLE is True:
        return  # already confirmed writable, skip repeated checks

    # Try /data first, then fallbacks
    candidates = [DATA_DIR] + _FALLBACK_DIRS
    for candidate in candidates:
        try:
            ...
            with open(test_path, "w") as f:
                f.write("ok")
            os.remove(test_path)
            # This directory works — use it
            if candidate != DATA_DIR:
                print(f"⚠️ /data not writable, using fallback: {candidate}")
                DATA_DIR = candidate
                DATA_FILE = os.path.join(candidate, "wallet_data.json")
            DATA_DIR_WRITABLE = True
            return
        except Exception as e:
            print(f"⚠️ Directory {candidate} not writable: {e}")
            continue
    DATA_DIR_WRITABLE = False
    print("❌ CRITICAL: No writable directory found! Data will NOT be saved.")
...
def save_data(data):
    ...
    try:
        ...
        # This PREVENTS corrupt/empty file if bot restarts mid-write
        tmp_file = DATA_FILE + ".tmp"
        with open(tmp_file, "w") as f:
            json.dump(data, f, indent=2)
            f.flush()
            os.fsync(f.fileno())  # force flush to disk before rename
        os.replace(tmp_file, DATA_FILE)  # atomic — never leaves partial file
...
def load_data():
    ...
    print("⚠️ No valid data file found, starting fresh.")
    return {
        "users": {},
        "transactions": [],
        "processed_tx": [],
...
def add_custom_service(provider_service_id, name, price, platform, min_qty, max_qty) -> str:
    ...
    with _DATA_LOCK:
        ...
        verify = load_data()
        saved = [s for s in verify.get("custom_services", []) if s["id"] == custom_id]
        if not saved:
            raise RuntimeError(f"Service {custom_id} was NOT saved to disk! Check /data volume.")
        return custom_id
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0, memo_code: str = None, method: dict = None):
    ...
    for THIS specific deposit request."""
    if not memo_code:
        return None
    transactions = await fetch_ton_native_transactions()
    memo_clean = memo_code.strip().upper()

    for tx in transactions:
        in_msg = tx.get("in_msg", {}) or {}
        tx_memo = extract_ton_comment(in_msg).strip().upper()
        if tx_memo != memo_clean:
            continue
        ...
        if min_amount and token_amount + 1e-9 < min_amount:
            # Amount too low — but still tell user (don't silently skip)
            print(f"⚠️ Deposit found for memo {memo_code} but amount too low: {token_amount} < {min_amount}")
            continue
        ...
        if live_price:
            ...
            usd_amount = round(token_amount, 3)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        # Keep pending deposit alive — no attempt limit, user can keep trying
        if not result:
    ...
    if data == "admin_back":
        ...
        try:
            ...
            lines = [
                ...
                f"🛍 Custom services saved: {len(d.get('custom_services', []))}",
            ]
            if last_processed:
...
async def admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # admin_action is cleared only after each action succeeds below
    ...
    try:
        if action.startswith("set_text:"):
            ...
            if not new_service:
                ...
                platform = text_in.strip().lower()  # normalize: "Telegram" → "telegram"
