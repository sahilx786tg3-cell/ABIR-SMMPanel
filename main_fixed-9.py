...
async def find_provider_service(service_id: str):
    ...
    services, err = await get_provider_services(force_refresh=True)  # always fresh fetch
    if err and not services:
        return None, err
    sid = str(service_id).strip()
    # First pass: exact ID match (handles int/str mismatch)
    for s in services:
        if str(s.get("service", "")).strip() == sid:
            return s, None
    # Second pass: case-insensitive name match (in case admin typed name instead of ID)
    sid_lower = sid.lower()
    for s in services:
        if sid_lower in str(s.get("name", "")).lower():
            return s, None
...
async def admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if action.startswith("set_text:"):
            ...
            svc, err = await find_provider_service(text_in.strip())
            if err and not svc:
                context.user_data["admin_action"] = None
                await update.message.reply_text(
